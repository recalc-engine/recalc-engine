//! `CHOOSECOLS` — return the specified columns from an array, in the given order.
//!
//! # Provenance
//! Behavior contract: Microsoft support "CHOOSECOLS function"
//! (<https://support.microsoft.com/en-us/office/choosecols-function-bf117976-2722-4466-9b9a-1c01ed9aebff>,
//! verified by WebFetch 2026-07-15). No `docs/specs/CHOOSECOLS.md` exists; this
//! is a clean-room implementation of the documented behavior only. The `_xlfn.`
//! authoring prefix is stripped by the parser
//! (`xl-ast::parser::parse_func_name`), so the registry name is bare
//! `CHOOSECOLS`. The column/row transpose of [`crate::func_chooserows`]; the
//! index validation is shared via [`crate::arrayshape::collect_indices`].
//!
//! # Behavior contract (one line)
//! `CHOOSECOLS(array, col_num1, [col_num2], …)` returns the listed columns of
//! `array`, in argument order.
//!
//! # Semantics implemented (MS page wording in parentheses)
//! - Column numbers are 1-based (Example 1). A negative column number counts
//!   from the end (Example 3).
//! - "Excel returns a #VALUE error if the absolute value of any of the col_num
//!   arguments is zero or exceeds the number of columns in the array."
//! - The chosen columns' cell values are relocated verbatim (structural select).
//!
//! # Refused / assumed edges — see `docs/plans/2026-07-15-lane3a-probe-needed.md`
//! Identical to [`crate::func_chooserows`]: unbounded/over-cap (L3A-CAP) and
//! array-valued / fractional / elided `col_num` (L3A-ARRIDX / L3A-FRAC), refused.

use xl_value::{ErrorKind, Value};

use crate::args::CallArgs;
use crate::arrayshape::{Materialized, collect_indices, materialize, over_cap, spill};
use crate::context::EvalContext;

/// Evaluate a `CHOOSECOLS(array, col_num1, …)` call. See the module docs for the
/// semantics and their spec provenance.
pub(crate) fn eval(_ctx: &EvalContext, args: &mut dyn CallArgs) -> Value {
    let grid = match materialize(args, 0) {
        Materialized::Grid(g) => g,
        Materialized::Omitted => return Value::Error(ErrorKind::Unsupported),
        Materialized::Refused(k) => return Value::Error(k),
    };
    let cols = match collect_indices(args, grid.cols) {
        Ok(c) => c,
        Err(v) => return v,
    };
    let num_cols = cols.len();
    if over_cap(grid.rows, num_cols) {
        return Value::Error(ErrorKind::Unsupported);
    }
    let mut data: Vec<Value> = Vec::with_capacity(grid.rows * num_cols);
    for r in 0..grid.rows {
        for &c in &cols {
            data.push(grid.at(r, c).clone());
        }
    }
    spill(grid.rows, num_cols, data)
}

#[cfg(test)]
mod tests {
    use super::eval;
    use crate::test_support::{TestArg, eval_direct, num};
    use xl_value::{Array, ErrorKind, Value};

    use TestArg::*;

    fn rect() -> TestArg {
        Rect {
            rows: 2,
            cols: 3,
            data: vec![num(1.0), num(2.0), num(3.0), num(4.0), num(5.0), num(6.0)],
        }
    }

    fn arr(rows: usize, cols: usize, data: Vec<Value>) -> Value {
        Value::Array(Array::new(rows, cols, data).unwrap())
    }

    // CHOOSECOLS(rect, 1, 3) → columns 1 and 3.
    #[test]
    fn choose_two_cols() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Scalar(num(1.0)), Scalar(num(3.0))]),
            arr(2, 2, vec![num(1.0), num(3.0), num(4.0), num(6.0)])
        );
    }

    // Columns are returned in the order requested (3 then 1).
    #[test]
    fn preserves_order() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Scalar(num(3.0)), Scalar(num(1.0))]),
            arr(2, 2, vec![num(3.0), num(1.0), num(6.0), num(4.0)])
        );
    }

    // Negative index counts from the end (-1 → last column).
    #[test]
    fn negative_index_from_end() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Scalar(num(-1.0))]),
            arr(2, 1, vec![num(3.0), num(6.0)])
        );
    }

    // |index| == 0 → #VALUE!.
    #[test]
    fn zero_index_is_value_error() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Scalar(num(0.0))]),
            Value::Error(ErrorKind::Value)
        );
    }

    // |index| exceeding the column count → #VALUE!.
    #[test]
    fn out_of_range_is_value_error() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Scalar(num(4.0))]),
            Value::Error(ErrorKind::Value)
        );
    }

    // L3A-ARRIDX: an array-valued index refuses.
    #[test]
    fn array_index_refused() {
        assert_eq!(
            eval_direct(eval, vec![rect(), Range(vec![num(1.0), num(2.0)])]),
            Value::Error(ErrorKind::Unsupported)
        );
    }
}
